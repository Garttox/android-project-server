<?php
// source: D:\www\a\app\presenters/templates/List/default.latte

use Latte\Runtime as LR;

class Template1dbe9f83e8 extends Latte\Runtime\Template
{
	public $blocks = [
...
?>
                                <a class="btn btn-dark" href="#"><i class="fas fa-qrcode"></i> QR</a>
<?php
			}
...
<?php
			if (($row['published'] == 'no' && $user->isInRole('admin'))) {
				?>                            <a class="btn btn-success right ajax" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("update!", [$row['id']])) ?>"><i class="fas fa-check"></i> Zveřejnit</a>
<?php
			}
			elseif (($row['published'] == 'yes' && $user->isInRole('admin'))) {
				?>                            <a class="btn btn-danger right ajax" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("update!", [$row['id']])) ?>"><i class="fas fa-times"></i> Stáhnout</a>
<?php
			}
			elseif (($row['published'] == 'wip' && $row['author'] == $user->getIdentity()->username)) {
				?>                            <a class="btn btn-secondary right ajax" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("update!", [$row['id']])) ?>"><i class="fas fa-flag-checkered"></i> Dokončit</a> 
<?php
			}
